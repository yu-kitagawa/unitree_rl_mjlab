...
def marker_pose_camera(
  env: ManagerBasedRlEnv, command_name: str
) -> torch.Tensor:
  """Actor marker pose using the current noisy localization estimate."""
  command = cast(GoalPoseCommand, env.command_manager.get_term(command_name))
  return command.localized_marker_pose_camera


def marker_pose_camera_ground_truth(
  env: ManagerBasedRlEnv, command_name: str
) -> torch.Tensor:
  """Ground-truth marker pose reserved for the privileged critic."""
  command = cast(GoalPoseCommand, env.command_manager.get_term(command_name))
  return command.marker_pose_camera
