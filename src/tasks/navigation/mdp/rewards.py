...
def constellation_reward(
  ...
  """Reward SE(2) goal alignment using equation (5) of Dugar et al.

  For a circular constellation, its planar moment of inertia is ``r**2`` and
  the mean squared point distance is exactly

  ``d_con = ||p - p*||^2 + 2 r^2 (1 - cos(theta - theta*))``.

  The returned reward is ``exp(-decay * d_con)``.  Translation and rotation
  are therefore coupled multiplicatively, as in the paper, instead of being
  tuned as independent additive rewards.
  """
  asset: Entity = env.scene[asset_cfg.name]
  command = cast(GoalPoseCommand, env.command_manager.get_term(command_name))

  position_error = (
    asset.data.root_link_pos_w[:, :2] - command.target_pos_w
  )
  position_distance_sq = torch.sum(torch.square(position_error), dim=-1)
  heading_error = torch.atan2(
    torch.sin(asset.data.heading_w - command.target_heading_w),
    torch.cos(asset.data.heading_w - command.target_heading_w),
  )
  orientation_distance = (
    2.0 * constellation_radius**2 * (1.0 - torch.cos(heading_error))
  )
  constellation_distance = position_distance_sq + orientation_distance

  env.extras["log"]["Metrics/navigation_position_error"] = torch.mean(
    torch.sqrt(position_distance_sq)
  )
  env.extras["log"]["Metrics/navigation_heading_error"] = torch.mean(
    torch.abs(heading_error)
  )
  env.extras["log"]["Metrics/constellation_distance"] = torch.mean(
...
def goal_stillness_reward(
  env: ManagerBasedRlEnv,
  command_name: str,
  lin_vel_std: float = 0.1,
  ang_vel_std: float = 0.2,
